...
export default function Dashboard() {
  ...
  const { atsScore, wordCount, fileName, breakdown, parsedText } = data;
            ...
            {isActive && (
              <div className="absolute left-0 right-0 top-full mt-2 z-40 w-full rounded-xl bg-[#12121a] border border-amber-500/40 p-4 shadow-2xl text-left text-[#f0f0f8] animate-fade-up">
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/8">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); handleCopyRewrite(weakBulletMatch.improved, weakBulletIdx); }}
        ...
        <div className="w-full lg:w-[48%] space-y-5 lg:max-h-[calc(100vh-110px)] lg:overflow-y-auto lg:pr-2">
          ...
          <div className="flex border-b border-white/6 overflow-x-auto">
            {[
              ...
              { id: "keywords", label: "Keywords Matrix", icon: TrendingUp },
              { id: "formatting", label: "Layout & Anchors", icon: AlertTriangle },
            ].map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                onClick={() => setActiveTab(id)}
                className={`flex items-center gap-1.5 border-b-2 px-4 py-2.5 text-xs font-bold transition-all whitespace-nowrap ${
                  activeTab === id
